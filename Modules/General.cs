...
{
    public class General : ModuleBase<CommandContext>
    {
        ...
        [Command("roll")]
        [Summary("Rolls an x sided die y times")]
        ...
        public async Task GetAv(string name = null)
        {
            ...
            if (name != null)
            {
                ...
                {
                    if (Extensions.ComputeLevenshteinDistance(username, user.Username) < Extensions.ComputeLevenshteinDistance(username, desiredUser.Username)) 
                        desiredUser = user as SocketUser;
                }
            }
